...
1. 突出显示并运行“**创建表值函数**”注释下的脚本。 此函数返回名为 enclosure_summary 的表。**** 通读函数代码，了解表的填充方式。

1. 突出显示并运行两个 select 语句，每次传入不同的机箱 ID。

1. 突出显示并运行“**如何使用具有 LATERAL 联接的表值函数**”注释下的部分。 该脚本展示了在联接中使用表值函数代替表单名称的用法。
...
你已成功完成此练习。 你已创建并执行几个存储过程。 你还创建了一个表值函数，并查看了 PostgreSQL 中提供的一些内置函数。

你现在可以在自己的 PostgreSQL 数据库中使用这些存储过程和函数，对数据执行多种操作。
